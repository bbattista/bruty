...
config = configs.read_config(r"D:\git_repos\bruty_dev_debugging\nbs\scripts\Barry.Gallagher.LA\bg_dbg.config", log_files=False,
                             base_config_path=r"D:\git_repos\bruty_dev_debugging\nbs\scripts\base_configs")['DEFAULT']
...
def fill_spec_tables_from_disk():
    ...
    tiles = cursor.fetchall()
    for i, (production_branch, locality, utm, hemisphere, datum, tile, resolution, datatype, for_nav, c_id, r_id, t_id) in enumerate(tiles):
        if i % 100 == 0:
